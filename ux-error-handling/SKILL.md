...
description: >
  Error handling UX: destructive action confirmation, undo, empty states,
  descriptive error messages, crash recovery, and server error handling. This
  is also the canonical home for empty-state standards. Use when designing or
  reviewing error states, confirmation dialogs, undo flows, or any screen that
...
| Empty state screens | Every empty screen follows the empty-state standards below. |
...
This is the canonical empty-state guidance for the whole skill set — other
skills (`ux-search`, `ux-visual-design`, `ux-help-onboarding`) reference it.

Every empty state has four parts:

1. **Visual** — an illustration or icon that matches the app's style.
2. **Heading** — short, positive, and specific ("No saved articles yet").
3. **Message** — one line explaining what this space is for.
4. **Action** — an optional primary button inviting the first step ("Browse articles").
...
| No search results | Pair "No results for X" with a spelling suggestion and related categories |
| Error-caused empty | Explain the failure and offer Retry — do not disguise an error as an empty state |
| Cleared / completed | Celebrate: "All caught up" with a positive visual |

Never leave a dead end: every empty state offers a next step or a path back.
...
- `ux-content` — error message tone and microcopy patterns
- `ux-network` — connectivity-specific error states
- `ux-forms` — inline validation and error recovery in forms
